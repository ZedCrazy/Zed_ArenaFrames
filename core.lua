...
function Zed_ArenaFrames:HideDefaultTexture()
local AEF = "ArenaEnemyFrame"
for i=1,5 do 
local frame = _G[AEF..i]
_G[AEF..i.."CastingBar"]:SetStatusBarTexture(TP_texture)
_G[AEF..i].healthbar:SetStatusBarTexture(TP_texture)
_G[AEF..i.."ManaBar"]:SetStatusBarTexture(TP_texture)


local NewClassIcon = CreateFrame("Button", frame:GetName().."NewClassIcon", frame, "SecureActionButtonTemplate")
local size = _G[AEF..i]:GetHeight()
NewClassIcon:SetSize(size,size)
NewClassIcon.texture = NewClassIcon:CreateTexture()
NewClassIcon.texture:SetAllPoints(NewClassIcon)
NewClassIcon.texture:SetTexture("")
NewClassIcon:Show()
NewClassIcon.unit = frame.unit

NewClassIcon.texture:SetTexture("Interface\\Glues\\CharacterCreate\\UI-CharacterCreate-Classes")
NewClassIcon.texture:SetTexCoord(unpack(CLASS_ICON_TCOORDS["DEATHKNIGHT"]))
frame.NewClassIcon = NewClassIcon
local _,selfClass = UnitClass(NewClassIcon.unit)
if selfClass then 
		NewClassIcon.texture:SetTexCoord(unpack(CLASS_ICON_TCOORDS[selfClass]))
	end 
NewClassIcon:SetPoint("CENTER", frame.classPortrait)
frame.NewClassIcon = NewClassIcon
------------- Update function ------------
frame:HookScript("OnShow", function(self)
local _,selfClass = UnitClass(self.unit)
	if selfClass then 
		self.NewClassIcon.texture:SetTexCoord(unpack(CLASS_ICON_TCOORDS[selfClass]))
	end 
end)
--------------------------------------------
------ BORDER FOR CLASS PORTRAIT ----------
local border = GetFrameBorder(frame.NewClassIcon, 3)

--------------------------------------------
--------------------------------------------
----------PET PORTRAIT ----------------------
local petframe = _G[AEF..i.."PetFrame"]
local petframe_portrait = _G[AEF..i.."PetFrame"]
--------------------------------------------
local NewPetPortrait = CreateFrame("Button", petframe:GetName().."NewPortrait", petframe, "SecureActionButtonTemplate")
NewPetPortrait:SetSize(petframe_portrait:GetSize())
NewPetPortrait.texture = NewPetPortrait:CreateTexture()
NewPetPortrait.texture:SetAllPoints(petframe_portrait)
NewPetPortrait.texture:SetTexture("")
NewPetPortrait:Show()
NewPetPortrait.unit = petframe.unit

--NewPetPortrait.texture:SetTexture(_G[AEF..i.."PetFrameTexture"]:GetTexture())
NewPetPortrait.texture:SetTexCoord(0.20, 0.90, 0.10, 0.90)

NewPetPortrait:SetPoint("CENTER", frame.classPortrait)
petframe.NewPetPortrait = NewPetPortrait
petframe.NewPetPortrait:Show()
--------------------------------------------
--------------------------------------------
---- UPDATE PET PORTRAIT -------------------
local function UpdatePetIcon(self)
local unit = self.unit 
if unit then local _,class = UnitClass(unit)
--SetPortraitTexture(self.texture, unit)
end 
end 
petframe:HookScript("OnShow", function(self)
UpdatePetIcon(self.NewPetPortrait)

end)
--------------------------------------------
--------------------------------------------
---------PET  BORDER---------------------
local border  = GetFrameBorder(NewPetPortrait,3)
--------------------------------------------
--------------------------------------------

--_G[AEF..i.."Texture"]:SetTexture("")
--_G[AEF..i.."PetFrameTexture"]:SetTexture("")
_G[AEF..i.."Texture"]:SetAlpha(0)
_G[AEF..i.."PetFrameTexture"]:SetAlpha(0)
end 
end 
--Zed_ArenaFrames:NewTextureTheme()
